...
def main():
    ...
    ap.add_argument(
        "--auto-apply",
        action="store_true",
        help="(optional) apply workspace changes back to repo (advanced)",
    )
    args = ap.parse_args()
    ...
    report_text = "\n".join(lines)
    report_path = args.report or write_report(
        report_dir,
        f"wkagent_report_{now()}.txt",
        report_text,
    )
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        f.write(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] report={report_path} repo={repo_path}\n"
        )
    ...
    if rc == 0:
        run(
            f'rsync -a --delete --exclude ".git" "{repo_path}/" "{str(ws_repo)}/"',
            cwd=".",
        )
    else:
    ...
    print(f"📦 Workspace repo path:\n  {ws_repo}\n")

    # Suggested next commands for user (copy-paste ready)
    repo_abs = repo_path  # already absolute
    ws_abs = str(ws_repo)

    print("➡️ Next steps (copy & paste):\n")
    print(f"1) Preview diff:\n   wkapply diff --repo \"{repo_abs}\" --ws \"{ws_abs}\"")
    print("")
    print("2) Apply (will create snapshot BEFORE apply; asks confirmation):")
    print(
        f"   wkapply apply --repo \"{repo_abs}\" --ws \"{ws_abs}\" --source wkagent --note \"codex-fix\""
    )
    print("")
    print("3) Apply without confirmation (dangerous, but handy):")
    print(
        f"   wkapply apply --repo \"{repo_abs}\" --ws \"{ws_abs}\" --source wkagent --note \"codex-fix\" --yes"
    )
    print("")
    print("4) If you want a manual restore point before experiments:")
    print(
        f"   wkapply snapshot --repo \"{repo_abs}\" --source manual --note \"before experiment\""
    )
    print("")
    print("5) Rollback (to latest snapshot):")
    print(f"   wkapply rollback --repo \"{repo_abs}\" --to latest")
    print("")
    print(
        "Tip: snapshots are in /opt/wktools/snapshots/<repo_name>/ and archives in /opt/wktools/archives/"
    )
