...
def show_diff(repo, workspace):
    ...
    for line in out.strip().splitlines():
        # "Files X and Y differ" или "Only in Y: file"
...
def main():
    ...
    ap.add_argument("--repo", required=True, help="Путь к репо")
    ap.add_argument("--workspace", required=True, help="Путь к workspace папке")
    ...
    # Get changed files
    changed = show_diff(repo, workspace)
    if not changed:
        sys.exit(0)

    print(f"\nИзменено файлов: {len(changed)}")
    for f in changed:
        rel = Path(f).relative_to(workspace) if workspace in f else f
        print(f"  • {rel}")
